...
function GestionMesas() {
    ...
    const [datosGlobales, setDatosGlobales] = useState(() => {
        const guardados = localStorage.getItem("comanda_mesas_v2");
    ...
    // Cargar restaurantes desde el localStorage 
    useEffect(() => {
        const guardados = JSON.parse(localStorage.getItem("comanda_restaurantes") || "[]");
        const nombres = guardados.length > 0 ? guardados.map(r => r.nombre) : ["La Bella Italia", "Parrillas Don Julio"];
    ...
    // Persistencia
    useEffect(() => {
        localStorage.setItem("comanda_mesas_v2", JSON.stringify(datosGlobales));
    }, [datosGlobales]);

    const mesasActuales = datosGlobales[restauranteActivo] || generarMesasIniciales();
    const filtradas = mesasActuales.filter(m => filtroZona === "Todas" || m.zona === filtroZona);

    const cambiarEstado = (id, nuevoEstado) => {
        const nuevasMesas = mesasActuales.map(m => m.id === id ? { ...m, estado: nuevoEstado } : m);
        setDatosGlobales({ ...datosGlobales, [restauranteActivo]: nuevasMesas });
                ...
                {filtradas.map(m => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <div style={{ fontSize: "2.5rem", marginBottom: "15px" }}>{m.capacidad >= 6 ? "🏯" : m.capacidad >= 4 ? "🍽️" : "☕"}</div>
