...
function Bookings() {
  const { reservas, cambiarEstadoReserva } = useTables();
  ...
  const obtenerConteo = (estado) =>
    reservas.filter(r =>
      r.restaurante === restauranteActivo &&
    ...
    <>
      <style>{`
        ...
        .rv-select-local {
          ...
          border: 2px solid #F4956A; font-weight: 700; outline: none;
          cursor: pointer; font-size: 0.88rem; font-family: inherit; background: white; color: #1a1a2e;
        }
        .rv-locked-badge {
          background: #fff8ee; color: #F4956A;
          border: 2px solid #F4956A; border-radius: 11px;
          padding: 9px 14px; font-weight: 700; display: flex; align-items: center; gap: 7px; font-size: 0.88rem;
        }
        ...
        .rv-stat-icon {
          ...
          display: flex; align-items: center; justify-content: center; margin-right: 13px; flex-shrink: 0;
        }
        ...
        /* Filtros */
        .rv-filtros {
          background: #f8f9fa; padding: 14px 16px; border-radius: 14px;
        ...
        .rv-date-input {
          padding: 7px 11px; border-radius: 9px; border: 1.5px solid #e8e0d8;
          font-size: 0.83rem; outline: none; font-weight: 600; font-family: inherit;
        }
        ...
        .rv-avatar {
          width: 46px; height: 46px; background: #f0f0f0; border-radius: 50%;
          display: flex; align-items: center; justify-content: center;
          font-size: 1.1rem; color: #1a1a2e; font-weight: 800; flex-shrink: 0;
        }
        ...
        .rv-info-cell {
          background: #fdfdfd; border: 1px solid #f0ebe4; border-radius: 10px;
        ...
        /* Notas */
        .rv-notas {
          background: #fffaf0; border: 1px dashed #ff9f22; border-radius: 11px;
          padding: 11px 14px; font-size: 0.8rem; color: #92400e; margin-bottom: 16px; line-height: 1.5;
        }

        /* Acciones */
        .rv-acciones { display: flex; gap: 8px; flex-wrap: wrap; }
        .rv-btn-confirmar {
          flex: 2; min-width: 120px; padding: 9px; border-radius: 11px; border: none;
          background: #22c55e; color: white; font-weight: 700; cursor: pointer; font-size: 0.8rem;
          display: flex; align-items: center; justify-content: center; gap: 5px;
          transition: opacity 0.2s;
        }
        .rv-btn-confirmar:hover { opacity: 0.88; }
        .rv-btn-cancelar-res {
          flex: 1; min-width: 80px; padding: 9px; border-radius: 11px; border: none;
          background: #fee2e2; color: #dc3545; font-weight: 700; cursor: pointer; font-size: 0.8rem;
          transition: opacity 0.2s;
        }
        .rv-btn-cancelar-res:hover { opacity: 0.82; }
        .rv-btn-anular {
          width: 100%; padding: 9px; border-radius: 11px; border: 1.5px solid #fee2e2;
          background: white; color: #dc3545; font-weight: 700; cursor: pointer; font-size: 0.8rem;
      ...
      <div className="rv-page">
        ...
        <div className="rv-header">
          <div>
            ...
            <p className="rv-subtitle">Gestiona las visitas para {restauranteActivo || "—"}</p>
          </div>
          <div className="rv-filtro-local">
            <span className="rv-filtro-label">FILTRAR POR LOCAL</span>
            {isAdmin ? (
              <select className="rv-select-local" value={restauranteActivo}
        ...
        <div className="rv-filtros">
          <div className="rv-filter-btns">
            {["todos", "pendiente", "confirmada", "cancelada", "cancelada_cliente"].map(f => (
              <button key={f} className={`rv-filter-btn ${filtroEstado === f ? "active" : ""}`}
          ...
          <div className="rv-date-group">
            ...
            <input type="date" className="rv-date-input" value={fechaFiltro}
              ...
              <div className="col-12 col-xl-6" key={r.id}>
                <div className="rv-card" style={{ borderLeftColor: ec.color }}>
                  ...
                  <div className="rv-acciones">
                    {r.estado === "pendiente" && (
                      <>
                        <button className="rv-btn-confirmar" onClick={() => cambiarEstadoReserva(r.id, "confirmada")}>
                          <i className="bi bi-check2-all"></i>Confirmar Reserva
                        </button>
                        <button className="rv-btn-cancelar-res" onClick={() => cambiarEstadoReserva(r.id, "cancelada")}>
                          Cancelar
                        </button>
                      </>
                    )}
                    {r.estado === "confirmada" && (
                      <button className="rv-btn-anular" onClick={() => cambiarEstadoReserva(r.id, "cancelada")}>
                        Anular Confirmación
                      </button>
                    )}
                  </div>
